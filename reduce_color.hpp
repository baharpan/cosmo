...
public:
  ...
  std::map<string,size_t> index_maker(){
    ...
    void build_recolored_matrix (){
      vector<string> files = this->load_input();
      std::map<string,size_t> pairs = this->index_maker();
      ...
      for(size_t i = 0; i < files.size(); i++){
        ...
        this->parser(ifstream(files[i]), fast[i]);
        this->kmer_counter (32, fast[i], found_kmer[i],pairs);
