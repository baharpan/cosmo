...
public:
  ...
  string bulge_type(size_t length, vector<std::string> &branch_labels){
    size_t i = 0;
    char first_var = branch_labels[i][0];
    i++;
    while (i < branch_labels.size()){
      if (first_var != branch_labels[i][0])
      break;
      i++;
    }
    vector<size_t> insertion;
    vector<size_t> deletion;
    for (size_t j = 1; j < length; j++){
      if (branch_labels[0][j] != branch_labels[i][j])  {
	       //if (branch_labels[0][j] == branch_labels[i][j+1] ) deletion.push_back(1);
	       //if (branch_labels[0][j] ==  branch_labels[i][j-1]) insertion.push_back(1);
	}

    if (deletion.size() > 29 || insertion.size() > 29) return ("indel");
    }
    return ("snp");
  }
    ...
    while ( dbg.outdegree(pos) != 0 && branch_labels[branch_num].size() < 1000 && !visited_cycle[pos]) {
      ...
      if (dbg.outdegree(pos) > 1 ) {
        ...
        if (visited[pos] && it != FoundBulge.end()) {
          ...
          if (!key) length = left_flank_embed.size();
          key = true;
          size_t num_copy = 0;
      ...
      if (bulge == true && branch_labels.size() > 1){
        ...
        cout<<"\n>Bulge number "<<bulge_count<<" From node number "<<i<</*" Type: "<< bulge_type(31, branch_labels)<<*/" variation of ";
